...
<<<<<<< HEAD
  const loadDashboardOverview = async () => {
    ...
    setView('overview');
    try {
      const vacs = await apiRequest(`/vacancies/all?id=${targetId}`);
      const list = Array.isArray(vacs) ? vacs : [];
      const stats = await Promise.all(
        list.map(async (job) => {
          const jobId = job.id || job.ID;
          let apps = [];
          try {
            apps = await apiRequest(`/vacancies/${jobId}/applications`);
          } catch (_) {}
          if (!Array.isArray(apps)) apps = [];
          const withScore = apps.filter(a => a.ai_score != null && a.ai_score !== undefined);
          const avgScore = withScore.length
            ? Math.round(withScore.reduce((s, a) => s + (Number(a.ai_score) || 0), 0) / withScore.length)
            : null;
          const matchCount = apps.filter(a => (Number(a.ai_score) || 0) >= 70).length;
          const matchPercent = apps.length ? Math.round((matchCount / apps.length) * 100) : null;
          return {
            id: jobId,
            title: job.title || job.Title,
            status: (job.is_archived || job.IsArchived) ? 'Archived' : 'Active',
        ...
        {view === 'overview' && (
          <div className="max-w-6xl mx-auto">
            <div className="mb-8">
              ...
              <p className="text-slate-500">Сводка по вакансиям, откликам и соответствию кандидатов</p>
            </div>
            {loading ? (
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">Вакансия</th>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">Статус</th>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">Откликов</th>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">Средний % соответствия</th>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">Соответствуют (≥70%)</th>
                      <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {dashboardStats.length === 0 ? (
                      <tr><td colSpan={6} className="px-6 py-16 text-center text-slate-400 font-medium">Нет вакансий или данных</td></tr>
                    ) : (
                      dashboardStats.map((row) => (
                        <tr key={row.id} className="hover:bg-slate-50/50 transition">
                          <td className="px-6 py-4 font-bold text-slate-900">{row.title}</td>
                          <td className="px-6 py-4"><StatusBadge status={row.status} /></td>
                          <td className="px-6 py-4 font-bold text-slate-700">{row.applicationsCount}</td>
                          <td className="px-6 py-4">
                            {row.avgScore != null ? (
                              <span className={row.avgScore >= 70 ? 'text-green-600 font-bold' : row.avgScore >= 50 ? 'text-amber-600 font-bold' : 'text-slate-600'}>{row.avgScore}%</span>
                            ) : (
                          ...
                          <td className="px-6 py-4">
                            {row.applicationsCount > 0 ? (
                              <span className="text-slate-700 font-medium">{row.matchCount} ({row.matchPercent}%)</span>
                            ) : (
                              <span className="text-slate-300">—</span>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            <button onClick={() => { const j = vacancies.find(v => (v.id || v.ID) === row.id) || { id: row.id, title: row.title }; handleJobClick(j); }} className="text-blue-600 font-bold text-sm hover:underline">Отклики →</button>
                          </td>
                        </tr>
                      ))
                    )}
